...
Element.generic_dp({
	...
	size: 	- The expected dimension of image to crop
			...
			- default is '300x300'.
	backend: full directory to the generic backend,
	Folder_p : 	-Folder where uploaded images go to, it would be created if it does not exist (dir with respect to processor file)
				-default is "asset/picture/";
	Folder_w : The same folder above but (dir with respect to current working page)
...
$.fn.extend({
	generic_dp_resizable : function(data){
		...
		$(this).find("#resizable").resizable({
			containment:"#generic_dp .new_container", aspectRatio: wt / ht})
		.draggable({
			containment:"#generic_dp .new_container", cursor: "move"}).tooltip({
...
$.fn.extend({
	generic_dp_init : function(init){
		"use strict";
		init.default = init.backend+"images/c_icon.png";
		init.processor = init.backend+"upload_v2_0.php";
		init.olds = [];
		$(this)[0].param = init;
		$(this).find('#inputImage').on('change',function(){
			...
			fm.append('folder', param.Folder_p);
			if(current){
				var cur_c = current.replace(param.Folder_p, param.Folder_w);
			...
			$.ajax({//just upload
				...
				if(!data){console.log(respon); toast("An Error Occured",'red');}else{
					if(data.error){
						...
						gdp.find('#user_image').attr({'data-old':current, 'data-img':param.Folder_p+data.name});
						gdp.find('#user_image').attr({'src':param.Folder_w+data.name});
	...
	generic_dp_delete : function(clear){
		...
		$.ajax({
			...
			if(!data){
				...
				if(clear !== false){image.removeAttr('data-src, data-img').attr({src:param.default});}
	...
	$.ajax({
		...
		if(!data){console.log(response); toast("An Error Occured",'red');}else{
			if(data.error){
				...
				generic_dp.find('#user_image').removeAttr('src','data-img').attr({'src':param.Folder_w+data.name,'data-img':param.Folder_p+data.name});
				...
				if(typeof(param.complete) === 'function'){
					var callback = param.complete;
					callback({src:param.Folder_w+data.name});
				}
			}
		}
	}).fail(function(){
